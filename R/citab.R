...
#' @param events Character vector for event names (only include state 0 if state0 not NA).
#' @param evlabs Character vector for event labels (only include state 0 if state0 not NA).
...
#' @param state0 Character event name for state 0. Default = NA (do not display estimates for state 0).
...
citab   <- function(msfit,
    ...
    if (grouped){
        ...
        res$strata <- factor(as.character(res$strata),
                             levels = groups_eq,
                             labels = grlabs)
        res <- res[order(as.numeric(res$strata), res$order),]
